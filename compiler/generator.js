  ...
  function evaluateNode(node, context) {
    switch (node.type) {
      ...
      case "Define":
        ...
        if (enableLogs)
          console.log(`Evaluating Define: ${node.id} = ${context[node.id]}`);
        break;
      case "Show":
      ...
      case "Identifier":
        if (node.name in context) {
          if (enableLogs)
            console.log(
              `Evaluating Identifier: ${node.name} = ${context[node.name]}`
            );
      ...
      case "BinaryExpression":
        ...
        if (enableLogs)
          console.log(
            `Evaluating BinaryExpression: ${left} ${node.operator} ${right}`
          );
      ...
      case "When":
        ...
        let iterations = 0;
        while (
          evaluateNode(node.condition, context) &&
        ...
        if (iterations === MAX_ITERATIONS) {
          console.warn(
            "Maximum iterations reached. Loop terminated to prevent infinite loop."
          );
        }
        break;
      case "Increment":
        if (node.argument.type === "Identifier") {
          ...
          if (enableLogs)
            console.log(
              `Incrementing ${node.argument.name} to ${
                context[node.argument.name]
              }`
            );
        }
      ...
      case "Decrement":
        if (node.argument.type === "Identifier") {
          ...
          if (enableLogs)
            console.log(
              `Decrementing ${node.argument.name} to ${
                context[node.argument.name]
              }`
            );
        }
        break;
      case "Condition":
        if (enableLogs) console.log("Evaluating Condition");
        if (evaluateNode(node.condition, context)) {
          node.body.forEach((n) => evaluateNode(n, context));
        } else if (node.elseBody) {
          node.elseBody.forEach((n) => evaluateNode(n, context));
        }
        break;
      case "Assignment":
        context[node.id] = evaluateNode(node.value, context);
        if (enableLogs)
          console.log(
            `Evaluating Assignment: ${node.id} = ${context[node.id]}`
          );
        break;
