function tokenize(input, enableLogs = false) {
  ...
  while (current < input.length) {
    ...
    if (char === '-') {
        // Look ahead to see if it's a number
        numberRegex.lastIndex = current;
        const possibleNumber = numberRegex.exec(input);
        if (possibleNumber && possibleNumber[0].startsWith('-')) {
            tokens.push({ type: 'number', value: possibleNumber[0], position: current });
            if (enableLogs) console.log(`Token: ${possibleNumber[0]} (number)`);
            current += possibleNumber[0].length;
            continue;
        }
    ...
    if (char === "?") {
      tokens.push({ type: "question", value: char, position: current });
      if (enableLogs) console.log(`Token: ${char} (question)`);
      current++;
      continue;
    }

    if (char === ":") {
      tokens.push({ type: "colon", value: char, position: current });
      if (enableLogs) console.log(`Token: ${char} (colon)`);
      current++;
      continue;
    }
   // Updated part to handle numbers and negative numbers
   numberRegex.lastIndex = current;
   const numberMatch = numberRegex.exec(input);
   if (numberMatch) {
       tokens.push({ type: 'number', value: numberMatch[0], position: current });
       if (enableLogs) console.log(`Token: ${numberMatch[0]} (number)`);
